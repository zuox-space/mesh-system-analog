...
export async function GET() {
  ...
  try {
    ...
    const scheduleCacheKey = `schedule:${teacherId}:year`;
    let schData = cacheGet<any>(scheduleCacheKey);
    ...
    if (!schData) {
      ...
      console.log("[ktp/groups] schedule cached");
    } else {
      console.log("[ktp/groups] schedule from cache");
    }
    ...
    const plansCacheKey = `plans:${teacherId}:${groupIds.join(",")}`;
    let plansData = cacheGet<any>(plansCacheKey);
    ...
    if (!plansData) {
      ...
      if (!plansRes.ok) {
        console.warn("[ktp/groups] calendar_plans failed:", plansRes.status);
        const groups: GroupSummary[] = [...groupsMap.values()];
        return NextResponse.json({ ok: true, academicYear, groups });
      }

      plansData = await plansRes.json();
      cacheSet(plansCacheKey, plansData, TTL.PLANS);
      console.log("[ktp/groups] plans cached");
    } else {
      console.log("[ktp/groups] plans from cache");
    }
    ...
    for (let i = 0; i < planIds.length; i += chunkSize) {
      ...
      const results = await Promise.all(
        chunk.map(async (pid) => {
          const planCacheKey = `plan:${pid}`;
          const cachedPlan = cacheGet<any>(planCacheKey);

          if (cachedPlan) {
            return { pid, ...countTopics(cachedPlan) };
          }
          ...
          try {
            ...
            if (!r.ok) {
              console.warn(`[ktp/groups] plan ${pid} failed: ${r.status}`);
              return { pid, total: 0, withDate: 0, withoutDate: 0 };
            }
            const d = await r.json();
            cacheSet(planCacheKey, d, TTL.PLAN_DETAILS);
            return { pid, ...countTopics(d) };
          } catch (e) {
            console.error(`[ktp/groups] plan ${pid} error:`, e);
            return { pid, total: 0, withDate: 0, withoutDate: 0 };
          }
        })
      );
    ...
    for (const g of groupsMap.values()) {
      ...
      g.has_issues = g.plan_topics_without_date > 0;
