// src/app/api/homework/set-empty/route.ts
...
export async function POST(req: Request) {
  ...
  if (!group_id || !subject_id || !date_assigned_on || !date_prepared_for) {
    return NextResponse.json(
      {
        detail:
          "Нужны group_id, subject_id, date_assigned_on, date_prepared_for",
      },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    cacheInvalidate("presence:");

    await prisma.logEntry.create({
      data: {
        userId: user.id,
        action: "HOMEWORK_SET_EMPTY",
