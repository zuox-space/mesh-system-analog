...
async function autoRefreshAll() {
  ...
  try {
    const res = await grabTokenFromTab();
    if (res?.success) {
      console.log("[bg] autoRefreshAll: токен обновлён");
    } else if (res?.skipped) {
      console.log("[bg] autoRefreshAll: токен не обновлён —", res.reason);
      await checkToken();
    } else {
      console.log("[bg] autoRefreshAll: токен не получен —", res?.error || "unknown");
      await checkToken();
    }
  } catch (e) {
    console.log("[bg] autoRefreshAll: grabToken error:", e.message);
    await checkToken();
  }
  ...
  try {
    ...
    console.warn("[bg] autoRefreshAll: data refresh error:", e);
  }

  await notifySchoolTabs();
  console.log("[bg] autoRefreshAll: done");
}
...
async function grabTokenFromTab() {
  let tabs = await chrome.tabs.query({ url: "https://school.mos.ru/*" });

  let openedByUs = false;
  let tempTabId = null;

  if (!tabs.length) {
    console.log("[bg] grabTokenFromTab: school.mos.ru не открыт — открываю фоновую вкладку");
    ...
    try {
      ...
      await waitForTabLoad(tempTabId, 15000);
      await new Promise((r) => setTimeout(r, 1500));

      tabs = [{ id: tempTabId }];
    } catch (e) {
      console.warn("[bg] grabTokenFromTab: не удалось открыть вкладку:", e);
      if (openedByUs && tempTabId) {
        try { await chrome.tabs.remove(tempTabId); } catch { }
      }
      return { success: false, skipped: true, reason: "не удалось открыть school.mos.ru" };
    }
  }
      ...
      func: () => {
        ...
        for (const c of document.cookie.split(";")) {
          ...
          if (value.startsWith("eyJ") && value.length > 100) {
            return { success: true, token: value, source: "cookie." + name.trim() };
          }
        }
        return { success: false, error: "Токен не найден в cookies" };
      }
    });
    r = results?.[0]?.result;
  } catch (e) {
    r = { success: false, error: "executeScript: " + e.message };
  }
  ...
  if (r?.success && r.token) {
    ...
    await setState({
      meshToken: r.token,
      meshTokenSource: r.source,
      meshTokenExpiresAt: expiresAt
    });
    await checkToken();
  ...
  try {
    ...
    const launchStatusMap = new Map(
      (oldSchedule || []).map((l) => [`${l.date}|${l.time}|${l.groupId}`, {
        status: l.status, is_launched: l.is_launched, startedAt: l.startedAt, late: l.late, launchUrl: l.launchUrl
      }])
    );
...
async function notifySchoolTabs() {
  try {
    ...
    for (const tab of tabs) {
      try {
        await chrome.tabs.sendMessage(tab.id, { type: "refreshBadge" });
      } catch { /* ок */ }
    }
  } catch (e) {
    console.warn("[bg] notifySchoolTabs failed:", e);
  }
}
